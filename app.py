...
def inicializar_bd():
    ...
    # Tabla de Historial de Pagos (Recibos e Ingresos por Mora)
    cursor.execute("""
...
# --- TÍTULO PRINCIPAL ---
st.title("💰 Sistema de Préstamos LuisAnth v2.0")
st.write("Estructura financiera blindada con control de caja, moras y recibos.")
st.markdown("---")
...
# ==========================================
# 1. PANTALLA: PANEL FINANCIERO (DASHBOARD)
# ==========================================
if opcion == "📊 Panel Financiero":
    ...
    # Calcular ingresos totales por mora cobrada
    ...
    col2.metric("Dinero en Calle (Saldos)", f"${dinero_en_calle:,.2f}", delta_color="inverse")
...
# ==========================================
# 4. PANTALLA: REGISTRAR COBRO / PAGO (CON RECIBO WHATSAPP)
# ==========================================
    ...
    if not contratos_pendientes:
        ...
        if st.button("Procesar Cobro e Historial"):
            ...
            # Generar Recibo de Texto Dinámico para copiar
